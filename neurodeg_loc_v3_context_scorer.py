...
class EEGScorer(nn.Module):
    def __init__(self):
        ...
        self.gat_supervised_plv_recon = GAT(
            in_channels=19,
            hidden_channels=5,
            out_channels=1,
            num_layers=2,
    ...
    def forward(self, eeg_graph, mmse):
        ...
        pred_adj_matrix = torch.matmul(gin_scored_norm, gin_scored_norm.T)
        
        regional_scores = self.gat_supervised_plv_recon(pred_adj_matrix)
        
        if torch.eq(mmse, 30):
            mmse_conditioned = torch.zeros_like(regional_scores)
        return mmse_conditioned
